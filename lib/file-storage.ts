...
function createDatePath(now: Date) {
...
export async function persistPdfFile(fileName: string, type: string | null, content: Uint8Array) {
  ...
  const datePath = createDatePath(new Date())
  const targetDir = join(STORAGE_ROOT, uploadType, datePath.year, datePath.month)
  ...
  return {
    ...
    publicPath: `${PUBLIC_BASE_URL}/${uploadType}/${datePath.year}/${datePath.month}/${storedName}`.replace(/\/+/g, '/'),
  }
}
