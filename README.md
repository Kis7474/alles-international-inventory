...
## 로그인/권한 적용 현황 (PR #92 후속)

### 1) 로그인 선행 진입
- `/login`을 제외한 모든 페이지는 세션 쿠키가 없으면 로그인 화면으로 이동합니다.
- 모든 API 요청도 로그인 세션이 없으면 `401 Unauthorized`를 반환합니다.

### 2) 역할별 접근 차등
- 역할은 `ADMIN`, `STAFF` 2개입니다.
- `STAFF`는 `유니패스 설정(/settings/unipass)` 메뉴가 보이지 않으며 시스템 설정 API 접근이 차단됩니다.
- `sales` API는 STAFF 계정에서 본인 담당(`salespersonId`) 데이터만 조회/등록/수정/삭제할 수 있습니다.

### 3) 초기 계정/비밀번호 및 변경
초기 시드 계정(총 4개):

| 이름 | 이메일(ID) | 역할 | 초기 비밀번호 |
|---|---|---|---|
| BS | `bs@alles.local` | ADMIN | `SEED_ADMIN_PASSWORD` |
| SJ | `sj@alles.local` | ADMIN | `SEED_ADMIN_PASSWORD` |
| YR | `yr@alles.local` | STAFF | `SEED_STAFF_PASSWORD` |
| IK | `ik@alles.local` | STAFF | `SEED_STAFF_PASSWORD` |

- 실제 값은 `.env`의 `SEED_ADMIN_PASSWORD`, `SEED_STAFF_PASSWORD`를 따릅니다.
- 로그인 후 `설정 > 비밀번호 변경` 메뉴에서 비밀번호를 직접 변경할 수 있습니다.

## 레거시 정리/저장공간 최적화 시작점
- 업로드 파일을 `타입/연/월` 경로로 분리 저장하도록 변경해 디렉터리 당 파일 과밀을 줄였습니다.
  - 예시: `/uploads/import-export/2026/02/<uuid>.pdf`
- 1차 정리 대상(레거시):
  1. 미사용 문서/스크립트 목록화
  2. API별 인증 가드 누락 구간 일괄 적용
  3. 첨부파일 orphan(참조 없는 파일) 정리 스크립트 추가
