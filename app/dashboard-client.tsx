        ...
        <div className="bg-white p-4 md:p-6 rounded-lg shadow">
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="min-w-full">
              ...
              <tbody>
                {(data?.salespersonStats ?? []).map((stat: OverviewData['salespersonStats'][0], idx: number) => {
                  const marginRate = stat.totalSales > 0 ? (stat.totalMargin / stat.totalSales) * 100 : 0
                  return (
                    <tr key={idx} className="border-b">
                      <td className="py-2 text-gray-900">
                        {stat.salesperson?.name}
                        {(stat.salesperson?.commissionRate ?? 0) > 0 && (
                          <span className="ml-2 text-xs text-blue-600">(커미션 {((stat.salesperson?.commissionRate ?? 0) * 100)}%)</span>
                        )}
                      </td>
        ...
        <div className="bg-white p-4 md:p-6 rounded-lg shadow">
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="min-w-full">
              ...
              <tbody>
                {(data?.categoryStats ?? []).slice(0, 10).map((stat: OverviewData['categoryStats'][0], idx: number) => (
                  <tr key={idx} className="border-b">
      ...
      <div className="bg-white p-4 md:p-6 rounded-lg shadow">
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="min-w-full">
            ...
            <tbody>
              {(data?.recentTransactions ?? []).map((tx: OverviewData['recentTransactions'][0]) => (
